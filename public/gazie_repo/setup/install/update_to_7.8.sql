...
ALTER TABLE `gaz_XXXtesbro`	ADD COLUMN `id_orderman` INT(9) NOT NULL DEFAULT '0' AFTER `id_con`;
CREATE TABLE `gaz_XXXorderman` ( `id` INT(9) NOT NULL AUTO_INCREMENT, `order_type` VARCHAR(3) NULL DEFAULT NULL, `description` TEXT NOT NULL, `add_info` VARCHAR(80) NULL DEFAULT NULL, `id_tesbro` INT(9) NOT NULL, `adminid` VARCHAR(20) NOT NULL DEFAULT '',	`last_modified` TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP, PRIMARY KEY (`id`)) COLLATE='utf8_general_ci' ENGINE=MyISAM;
-- STOP_WHILE ( questo e' un tag che serve per istruire install.php a SMETTERE di eseguire le query su tutte le aziende dell'installazione)